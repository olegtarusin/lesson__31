 ...
 */            (function(window, factory) {
    ...
    (() => {
        ...
        document.addEventListener("touchstart", addClicked);
        document.addEventListener("touchend", addClicked);
        function addClicked(event) {
            const tapTarget = event.target;
            const parentClicked = document.querySelector(".item.clicked");
            if (parentClicked && !tapTarget.closest(".item.clicked")) parentClicked.classList.remove("clicked");
            if (tapTarget.closest(".item")) {
                const parent = tapTarget.closest(".item");
                if (event.type == "touchstart") {
                    parent.classList.add("clicked");
                    tapTarget.classList.add("clicked");
                }
                if (event.type == "touchend") tapTarget.classList.remove("clicked");
            } else {
                if (event.type == "touchstart") tapTarget.classList.add("clicked");
                if (event.type == "touchend") tapTarget.classList.remove("clicked");
            }
        }
        window.addEventListener("load", windowload);
        const cardWrapper = document.querySelector(".bord__body");
        const navWrapper = document.querySelector(".menu__list");
        function windowload() {
            if (cardWrapper && navWrapper) loadcard();
        }
        async function loadcard() {
            const pathData = "files/data.json";
            const response = await fetch(pathData, {
                metod: "GET"
            });
            if (response.ok) {
                const resolt = await response.json();
                const data = resolt.items;
                setItem(data);
                let groupList = [];
                for (let i = 0; i < data.length; i++) groupList[i] = data[i].group;
                const sortList = clonEater(groupList);
                setnav(sortList);
                groupSortData(data, sortList);
        ...
        function setItem(base) {
            let template;
            base.forEach((item => {
                template = `\n        <div class="bord__item item" id="${item.id}"->\n        <figure class="item__image">\n            <img src="${item.image}" alt="item">\n            <figcaption class="item__hide">\n                <a href="#" class="item__button">ADD TO CART <span class="item__icon icon--bag"></span></a>\n            </figcaption>\n        </figure>\n        <article class="item__info">\n            <h3 class="item__name">${item.name}</h3>\n            <div class="item__price">\n                <span>₦</span>${item.prise}\n            </div>\n        </article>\n    </div>`;
                cardWrapper.insertAdjacentHTML("beforeend", template);
            }));
        }
        function setnav(list) {
            let navTemplate = `<li id="all" class="menu__item"><a href="#" class="menu__link">All</a></li>`;
            navWrapper.insertAdjacentHTML("beforeend", navTemplate);
            list.forEach((group => {
                navTemplate = `<li id="${group.slice(0, 4)}" class="menu__item"><a href="#" class="menu__link">${group}</a></li>`;
                navWrapper.insertAdjacentHTML("beforeend", navTemplate);
            }));
        }
        function clonEater(list) {
            let sortList = new Set(list);
            sortList = Array.from(sortList);
            return sortList;
        }
        function groupSortData(base, list) {
            list.forEach((listItem => {
                let sortData = [];
                base.forEach((baseItem => {
                    if (baseItem.group == listItem) sortData.push(baseItem);
                }));
                let button = document.getElementById(listItem.slice(0, 4));
                if (button) {
                    button.addEventListener("click", changeBody);
                    function changeBody() {
                        cardWrapper.innerHTML = "";
        ...
        let quntaty = 0;
        let quntatyCase = document.querySelector(".menu__button.icon--bag span");
        let quntatyTitle = document.querySelector(".popup__title span");
        function addToBag(e) {
            const clickTarget = e.target;
            if (clickTarget.closest(".item__button")) {
                quntaty = ++quntaty;
                quntatyCase.innerHTML = quntaty;
                quntatyTitle.innerHTML = quntaty;
                e.preventDefault();
                const addedItem = clickTarget.closest(".item");
                let clonItem = addedItem.cloneNode(true);
                clonItem.removeAttribute("id");
                clonItem.classList.remove("clicked");
                addAnima(clonItem, addedItem);
            }
        }
        function addAnima(flyobject, parent) {
            const info = parent.getBoundingClientRect();
            document.body.append(flyobject);
            flyobject.style.cssText = `\n    position: fixed;\n    z-index: 100;\n    width: ${info.width}px;\n    height: ${info.height}px;\n    top: ${info.top}px;\n    left: ${info.left}px;\n    opacity: 1;\n    visibility: visible;\n    //overflow: hidden;\n    transition: all 0.9s;\n    pointer-events: none;`;
            const bag = document.querySelector(".menu__button.icon--bag");
            if (bag) {
                function change() {
                    const infoBag = bag.getBoundingClientRect();
                    flyobject.style.cssText = `\n            position: fixed;\n            z-index: 100;\n            width: ${infoBag.width}px;\n            height: ${infoBag.height}px;\n            top: ${infoBag.top}px;\n            left: ${infoBag.left}px;\n            opacity: 1;\n            visibility: hidden;\n            transition: all 0.9s;\n            overflow: hidden;\n            pointer-events: none;`;
                }
                setTimeout(change, 0);
                function delobject() {
                    flyobject.remove();
                }
                setTimeout(delobject, 1e3);
            }
        }
        const script_form = document.forms.quantform;
        const input = script_form.quantinput;
        script_form.addEventListener("click", inputchange);
        function inputchange(e) {
            let qtarget = e.target;
            let qauntvalue = input.value;
            if (qtarget.closest(".quantity--plus")) qauntvalue = Number(input.value) + 1;
            if (qtarget.closest(".quantity--minus") && input.value != "1") qauntvalue = input.value - 1;
            totalfunc(qauntvalue);
        }
        function totalfunc(value) {
            let subprise = document.querySelector(".subprise");
            subprise.innerHTML = `<span>₦</span>${value * 1e4}</div>`;
        }
        window["FLS"] = true;
        isWebp();
        menuInit();
